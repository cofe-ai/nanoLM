...
def train(model, train_dataloader, test_dataloader, accelerator, lr_scheduler,
    ...
    while args.current_train_step <= args.total_steps:
        ...
        for batch_id, batch in enumerate(train_dataloader, start=1):
            if args.current_train_step > args.total_steps:
                break
            loss, stats = forward(model, batch)
            accelerator.backward(loss / args.grad_acc)
            train_averager.update(stats)

            #更新信息
            # pbar(batch_id, {'loss': loss.item()})

            if batch_id % args.grad_acc == 0:
