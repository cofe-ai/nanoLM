    ...
    def __call__(self, examples: List[Dict[str, np.ndarray]]) -> BatchEncoding:
        ...
        for k, v in examples[0].items():
            for i in range(len(examples)):
                examples[i][k] = np.array(examples[i][k])
            # a =  np.array([examples[i][k] for i in range(len(examples))])
        batch = BatchEncoding(
            {
...
def tokenize_function(examples, tokenizer, in_length):
    ...
    if len(concatenated_ids) == 983:
        concatenated_ids = concatenated_ids[:460,:]
    concatenated_ids = concatenated_ids[:1000,:]
...
@dataclass
class DataCollatorForLanguageModelingGPT:
    """
    ...
    def __call__(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        if self.mlm:
            ...
            labels = batch['data']["input_ids"].clone()
